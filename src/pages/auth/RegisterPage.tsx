  ...
  const onSubmit = async (data: RegisterForm) => {
    try {
      ...
      if (msg.toLowerCase().includes('already registered')) {
        addToast({ type: 'error', title: t('auth.errors.emailTaken') })
      } else {
        addToast({ type: 'error', title: t('errors.generic'), description: msg })
      }
    }
  }
    ...
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-6 py-12 text-center"
        ...
        <p className="text-sm max-w-xs" style={{ color: 'var(--color-text-secondary)' }}>
          {t('auth.emailVerificationDesc', { email: emailSent })}
        </p>
        <Link
          to="/auth/login"
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
          <div>
            ...
            />
            {errors.email && <p className="text-xs mt-1 text-red-400">{t('auth.errors.invalidEmail')}</p>}
          </div>
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                placeholder="••••••••"
                className="w-full h-12 px-4 pr-12 rounded-2xl text-sm outline-none"
                style={{
                  backgroundColor: 'var(--color-bg-elevated)',
            ...
            {errors.password && <p className="text-xs mt-1 text-red-400">{t('auth.errors.weakPassword')}</p>}
          </div>
          ...
          <div>
            ...
            />
            {errors.confirmPassword && <p className="text-xs mt-1 text-red-400">{t('auth.errors.passwordMismatch')}</p>}
          </div>
        ...
        <p className="text-center text-sm mt-6" style={{ color: 'var(--color-text-muted)' }}>
          {t('auth.haveAccount')}{' '}
          <Link to="/auth/login" className="font-medium" style={{ color: 'var(--color-accent)' }}>
            {t('auth.login')}
          </Link>
        </p>
      </div>
